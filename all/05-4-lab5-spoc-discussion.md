...
2. 系统调用的参数传递过程？
 ...
 >
 > 参数依次存储在edx、ecx、ebx、edi、esi中，由syscall函数打包成数组后传入具体的处理函数中。
...
1. 在do_execve中的的当前进程如何清空地址空间内容的？在什么时候开始使用新加载进程的地址空间？
 ...
 > exit_mmap, put_pgdir, mm_destroy

 > CR3设置成新建好的页表地址后，开始使用新的地址空间

2. 新加载进程的第一级页表的建立代码在哪？

load_icoe函数中调用的setup_pgdir函数
...
load_icode函数中修改了trapframe中eip，将其指向elf文件的入口函数，从而开始执行新进程。
...
do_fork函数调用的copy_mm、copy_thread函数。
...
执行fork后不立刻复制父进程的内存空间，当发生修改时再复制

1. 写时复制的页表在什么时候进行复制？共享地址空间和写时复制有什么不同？

发生修改时。贡献地址空间在写时不会发生复制，同样的数据不一定对应相同的虚拟地址。

1. 存在有多个（n>2）进程具有父子关系，且采用了COW机制的情况。这个情况与只有父子两个进程的情况相比，在设计COW时，需要注意的新问题是什么？有何解决方案？
