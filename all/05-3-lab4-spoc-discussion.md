...
do_fork中父进程返回值设为了子进程pid，子进程在copy_thread中context的eax被设为了0.

3. 内核线程initproc的第一次执行流程是什么样的？能跟踪出来吗？

执行init_main。

3. 分析线程切换流程，找到内核堆栈、页表、寄存器切换的代码位置。

proc_run函数修改全局变量current，切换内核栈、切换页表、调用switch_to切换线程。

3. 分析C语言中调用汇编函数switch_to()的参数传递位置。

prev->context的指针位于esp+4，next->context的指针位于esp+8.
