...
寄存器位于CPU内，访问速度最快、数量最少、成本较高。

高速缓存位于CPU内，访问速度比内存快、大小较小、成本较高，对软件透明。

内存访问速度比缓存慢，大小较大，成本较低。

外存访问速度最慢，大小较大，成本最低。

1. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

各层存储介质均得到充分利用、处于繁忙状态，但是系统整体没有任何部分出现瓶颈。

1. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

出现过，例如构造文档数据集中词汇的共现矩阵由于行列数较大而无法储存，解决方法是使用邻接表储存来压缩矩阵。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

覆盖技术是划分程序模块使得同时执行的模块不会使用同一块内存。主要工作是划分模块、确定模块间的共享关系。

1. 什么是交换技术？覆盖与交换有什么不同？

交换技术是指将进程整体的内存空间移入移出内存以贡献内存。覆盖通过事先决定模块间运行先后顺序来解决内存问题，而交换在运行中动态解决。

1. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

有需要可能用在程序所需要使用的内存总大小大于实际物理内存时。
...
时间局部性：最近被访问的内存接下来很可能被访问

空间局部性：最近被访问的内存附近的内存很可能被访问

分支局部性：程序在时间-空间内执行路径的预期只有几种可能

1. 如何提高程序执行时的局部性特征？

顺序访问内存以利用缓存（如矩阵乘法）、减少函数调用等

3. 排序算法的局部性特征？

  大部分排序算法都顺序访问数组，空间局部性较好，仅堆排序不是顺序访问，因此尽管和快速排序一样不需要额外空间、复杂度相同，但是使用不多。
...
虚拟存储是只将当前指令执行所需要的部分进程空间存放在物理内存中。区别在于不将当前进程全部地址空间存入内存，使用虚拟地址。好处在于能提供大于物理内存的进程地址空间，挑战在于需要地址转换。

1. 虚拟存储需要什么样的支持技术？

需要硬件支持虚拟地址转换，操作系统支持置换算法
...
虚拟页式存储指将内存分页，在需要页时动态加载，支持将页替换到磁盘。缺页中断处理可以在内存页缺失时加载页。
...
虚拟页式存储需要操作系统支持，以进行缺页处理。

### 8.6 缺页异常

1. 缺页异常的处理流程？

发生缺页异常后，异常处理程序在外存找到缺失的页面，换入内存中，修改对应页表项，返回重新执行缺页异常指令。


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

访存时间\*（1-缺页率）+缺页异常处理时间*缺页率
