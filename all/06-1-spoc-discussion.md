...
2. 在什么时候可以进行处理机调度？
 ...
 > 占用CPU运行的线程的主动放弃（退出、等待）（非抢占系统）

 > 中断请求被服务例程响应完成时，当前线程被抢占（时间片用完、高优先级线程就绪）（可抢占系统）
...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   - 先来先服务算法（FCFC、FIFO）：队列顺序即为进入顺序。优点简单，缺点平均等待时间波动较大、IO和CPU资源利用率较低
   - 短进程优先算法（SPN、SJF）：队列按执行时间排序，执行时间短的排前面。优点平均等待时间最短，缺点可能导致饥饿、需要预知未来
   - 最高响应比优先算法（HRRN）：选择就绪队列中响应比R值最高的进程。优点该淋了短进程优先算法，缺点不可抢占
...
2. 为什么短进程优先算法的平均周转时间最优？
 ...
 > 可以证明知进程优先算法的调度顺序的平均周转时间是最短的。随意换两个的位置时间只会变长
...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   - 时间片轮转算法（RR）：每一个时间片结束切换到下一个就绪线程。优点公平，缺点带来额外的上下文切换开销
   - 多级反馈队列算法（MLFQ）：将就绪队列划分为多个独立的子队列分别处理
   - 公平共享调度算法（FSS）：控制用户对系统资源的访问，确保公平
...
3. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   - 速率单调调度算法(RM, Rate Monotonic) ：通过周期安排优先级，周期越短优先级越高
   - 最早截止时间优先算法 (EDF, Earliest Deadline First) ：截止时间越早优先级越高
